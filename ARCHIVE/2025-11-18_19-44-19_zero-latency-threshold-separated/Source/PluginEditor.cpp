...
void QuadBlendDriveAudioProcessorEditor::resized()
{
    ...
    // Reserve bottom for visualizations - FULL WIDTH (proportional) - 4x original size
    auto oscilloscopeArea = bounds.removeFromBottom(static_cast<int>(400 * scale));  // 100 * 4
    bounds.removeFromBottom(spacing);
    auto meterArea = bounds.removeFromBottom(static_cast<int>(180 * scale));  // 90 * 2 (50% of 4x = 2x)
    bounds.removeFromBottom(spacing);
